...
//   }
   ...
   wander() {
      ...
      ay = - ay * scale;                  // Moves the fish in a horizontal direction
   ...
   seekFood(food_list) {
      ...
      let closestFoodCoord = new PVector();   
...
//         debugger;
   ...
   //
   // Modified to prevent sharks from colliding   --Jim
   //
   separate (fishes) {
...
export class Shark extends Fish {
   ...
   look_for_food(fish_list) {   // go through the list and see if we can find anything to eat
      ...
      if(food_distance <= vision_distance/2){
   ...
   constructor() {
     ...
     // debugger;
      ...
      for(j = -separation; j <= separation; j = j + separation) {
         ...
         z = Math.random()*40 - 20;   // randomize the distance of coral from camera
